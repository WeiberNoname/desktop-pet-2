...
function loadCustomModel(filePath) {
  ...
  try {
    ...
    loader.load(fileUrl, (gltf) => {
      ...
      // Auto-normalize scale and centering using bounding box
      ...
      // Scale model to a standardized height of 1.9 units
      const maxDimension = Math.max(size.x, size.y, size.z);
      const targetHeight = 1.9;
      const scaleFactor = targetHeight / (maxDimension || 1);
      
      // Create an inner group to offset position
      const innerGroup = new THREE.Group();
      innerGroup.add(model);
      innerGroup.position.y = -0.1;
      ...
      console.log('Successfully loaded and normalized custom model:', filePath);
    }, undefined, (error) => {
...
function animate() {
  ...
  if (animationState.type === 'interact') {
    ...
    // Smooth floating/bobbing up and down (applies to both custom and procedural)
